# Notifications
Python script to generate notifications on smartphone with Pushsafer

Tested on Android devices :
Create account/register on https://www.pushsafer.com/
Buy with PayPal min.1000 calls for only 0.99€ (  or even less for more calls ) 
<img width="597" height="178" alt="afbeelding" src="https://github.com/user-attachments/assets/3628d40b-3cf1-4538-8591-51313035b462" />

Install 'Pushsafer' on Android (this will be the 'app' generating the notifications called by your python program to pushsafer provider)

When registered at pushsafer.com : you receive private key which shall be used in the Python program
also , create the Android device ( which have the app intalled ) on your dashboard at pushsafer 
test it before starting with Python coding :
<img width="251" height="261" alt="afbeelding" src="https://github.com/user-attachments/assets/3921006a-7645-4491-ba66-cddb23f9f93a" />

